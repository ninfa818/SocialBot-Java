...
public class NumbersForContactAdapter extends CheckableChildRecyclerViewAdapter<NumbersForContactAdapter.ContactNameHolder, NumbersForContactAdapter.PhoneNumberHolder> {
    ...
    public ContactNameHolder onCreateGroupViewHolder(ViewGroup parent, int viewType) {
        if (SharedPreferencesManager.getThemeMode().equals(AppUtils.THEME_DARK)) {
            return new ContactNameHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.row_contact_picker_dark, parent, false));
        } else {
            return new ContactNameHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.row_contact_picker_light, parent, false));
        }
    }
    ...
    public PhoneNumberHolder onCreateCheckChildViewHolder(ViewGroup parent, int viewType) {
        if (SharedPreferencesManager.getThemeMode().equals(AppUtils.THEME_DARK)) {
            return new PhoneNumberHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.row_phone_number_dark, parent, false));
        } else {
            return new PhoneNumberHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.row_phone_number_light, parent, false));
        }
    }
    ...
    class ContactNameHolder extends GroupViewHolder {
        ...
        ContactNameHolder(View itemView) {
            super(itemView);
            tvContactNamePick = itemView.findViewById(R.id.tv_contact_name_pick);
        }

        void setContactName(ExpandableGroup group) {
    ...
    class PhoneNumberHolder extends CheckableChildViewHolder {
        ...
        PhoneNumberHolder(View itemView) {
            super(itemView);
