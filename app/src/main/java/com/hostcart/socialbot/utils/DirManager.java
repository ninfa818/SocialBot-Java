...
public class DirManager {
    ...
    ///////////////////////////////////////////////////////////////////////
    private static String postVideoPath = mainAppFolder() + "/" + APP_FOLDER_NAME + "/" + "postVideo/";
    private static String postImagePath = mainAppFolder() + "/" + APP_FOLDER_NAME + "/" + "postImage/";

    public static File isExistFile( String filename, int type ) {
        File file;

        if( type == 1 )
            file = new File(postImagePath + filename + EXTENSION_JPG);
        else
            file = new File(postVideoPath + filename + EXTENSION_MP4);
    ...
    public static void createDirectory( int type ) {
        File file;

        if( type == 1 )
            file = new File(postImagePath);
        else
            file = new File(postVideoPath);
    ...
    public static File createFile( String filename, int type ) {
        File file;
        ...
        if( type == 1 )
            ...
            file = new File(postVideoPath + filename + EXTENSION_MP4);
//        File file = new File(mainAppFolder() + "/" + APP_FOLDER_NAME + "/" + "postVideo/"
//                + filename + EXTENSION_MP4);
    ...
    public static String getReceivedPostFolder() {
        File file = new File(mainAppFolder() + "/" + "Post");
        if (!file.exists()) {
            file.mkdirs();
        }
        return file.getPath();
    ...
    public static File getReceivedPostFile(String localpath) {
        //if the status was downloaded from other users we will save it in the hidden folder ".Statuses"
        //otherwise we will save as a normal received image or video
        String filename = localpath.substring(localpath.lastIndexOf("/")+1);
        return new File(DirManager.getReceivedPostFolder() + "/" + filename);
