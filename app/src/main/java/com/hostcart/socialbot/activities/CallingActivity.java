...
public class CallingActivity extends AppCompatActivity implements CallingService.StartFailedListener, ServiceConnection {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (SharedPreferencesManager.getThemeMode().equals(AppUtils.THEME_DARK)) {
            setContentView(R.layout.activity_phone_call_dark);
            ...
            setContentView(R.layout.activity_phone_call_light);
            Window window = getWindow();
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        ...
        String phoneNumber = getIntent().getStringExtra(IntentUtils.PHONE);
        ...
        if (user != null) {
            ...
            if (user.getUserLocalPhoto() != null) {
                ...
                if (user.getThumbImg() != null) {
                    Glide.with(this).asBitmap().load(BitmapUtils.encodeImageAsBytes(user.getThumbImg())).error(R.mipmap.ic_launcher_round).into(imgUser);
                } else {
                    Glide.with(this).asBitmap().load(user.getPhoto()).error(R.mipmap.ic_launcher_round).into(imgUser);
                }
            }
        } else {
        ...
        btnAnswer.setOnClickListener(view -> answerCall());

        btnReject.setOnClickListener(view -> endCall());

        btnHangup.setOnClickListener(view -> endCall());
    ...
    private class UpdateCallDurationTask extends TimerTask {
        ...
        @Override
        public void run() {
            CallingActivity.this.runOnUiThread(CallingActivity.this::updateCallDuration);
        }
    }
